...
function LineChart({ params }) {
  ...
  const yScaleDomainPeriod = (UtilityFunctions.calculateYScaleDomain(filteredData, currentDrug, selectedDrugs, currentState, currentDrugOnly) * 1.2);
                  ...
                  {Object.keys(inp.filteredData).map(key => <Group key={`line-path-${key}`}>
                    {specs.xValues.map((xVal, i) => {
                      ...
                      return (
                        <Group key={`line-path-${key}-point-${i}`}>
                          {(!isNaN(d[currentDrug]) && !isNaN(dNext[currentDrug]) && key == 'US' && currentDrugOnly) && 
                            <line x1={specs.xScale(d[specs.xKey]) ?? 0} y1={specs.yScale(d[currentDrug]) ?? 0} x2={specs.xScale(dNext[specs.xKey]) ?? 0} y2={specs.yScale(dNext[currentDrug]) ?? 0} stroke={UtilityFunctions.getSeriesColor(currentDrug, key, currentDrugOnly)} strokeWidth={3} />
                          }
                          {(!isNaN(d[currentDrug]) && !isNaN(dNext[currentDrug]) && key != 'US') && 
                            <line x1={specs.xScale(d[specs.xKey]) ?? 0} y1={specs.yScale(d[currentDrug]) ?? 0} x2={specs.xScale(dNext[specs.xKey]) ?? 0} y2={specs.yScale(dNext[currentDrug]) ?? 0} stroke={UtilityFunctions.getSeriesColor(currentDrug, key, currentDrugOnly)} strokeWidth={3} />
                          }
                          {(!isNaN(d[currentDrug]) && key == 'US' && currentDrugOnly) && <text x={i == 0 ? specs.xScale(d[specs.xKey]) :  specs.xScale(d[specs.xKey])} y={specs.yScale(d[currentDrug])-8} stroke={''} fill={''} fontSize={12} textAnchor={i == 0 ? 'right' : 'middle'}>{showLabels ? d[currentDrug] : ''}</text>}
                          ...
                          {(!isNaN(d[currentDrug]) && key == 'US' && currentDrugOnly) && <Circle onMouseEnter={(event) => {percentfunc(currentDrug, d[specs.xKey], d[currentDrug], dPrev[specs.xKey], dPrev[currentDrug], inp.stateNames[key], d['year'])}} cx={specs.xScale(d[specs.xKey])} cy={specs.yScale(d[currentDrug])} r={4} fill={currentTimeframe === 'Monthly' && d[specs.xKey] == currentMonth ? 'orange' : UtilityFunctions.getSeriesColor(currentDrug, key, currentDrugOnly)} />}
                    ...
                    {(!specs.isSmallViewport && yScaleDomainPeriod > 0) &&  (() => {
                        ...
                        return <text 
                          ...
                          fill={UtilityFunctions.getSeriesColor(currentDrug, key, currentDrugOnly)}>
                            {key != 'US' ? inp.stateNames[key] : currentDrugOnly ? inp.stateNames[key] : ''}
                        </text>
                      })()
                    }
                  </Group>)}
          ...
          <td style={{width: '85%'}}>
            <svg style={{height: specs.height, width: showPercent ? specs.width : '100%'}}>
              <Group top={specs.margin.top} left={specs.margin.left}>
                ...
                {inp.selectedDrugs.includes('alldrug') && currentDrug != 'alldrug' && currentState === 'US' && buildLineForDrug(specs, data, inp, 'alldrug', showLabels, showPercent, isPeriod)}
                {inp.selectedDrugs.includes('opioid') && currentDrug != 'opioid' && currentState === 'US' && buildLineForDrug(specs, data, inp, 'opioid', showLabels, showPercent, isPeriod)}
                {inp.selectedDrugs.includes('heroin') && currentDrug != 'heroin' && currentState === 'US' && buildLineForDrug(specs, data, inp, 'heroin', showLabels, showPercent, isPeriod)}
                {inp.selectedDrugs.includes('stimulant') && currentDrug != 'stimulant' && currentState === 'US' && buildLineForDrug(specs, data, inp, 'stimulant', showLabels, showPercent, isPeriod)}
                {inp.selectedDrugs.includes('benzo') && currentDrug != 'benzo' && currentState === 'US' && buildLineForDrug(specs, data, inp, 'benzo', showLabels, showPercent, isPeriod)}
                {inp.selectedDrugs.includes('fentanyl') && currentDrug != 'fentanyl' && currentState === 'US' && buildLineForDrug(specs, data, inp, 'fentanyl', showLabels, showPercent, isPeriod)}
                {inp.selectedDrugs.includes('cocaine') && currentDrug != 'cocaine' && currentState === 'US' && buildLineForDrug(specs, data, inp, 'cocaine', showLabels, showPercent, isPeriod)}
                {inp.selectedDrugs.includes('methamphetamine') && currentDrug != 'methamphetamine' && currentState === 'US' && buildLineForDrug(specs, data, inp, 'methamphetamine', showLabels, showPercent, isPeriod)}
                <AxisLeft
                  scale={specs.yScale}
                  tickLabelProps={() => ({
                    fontSize: specs.fontSize,
