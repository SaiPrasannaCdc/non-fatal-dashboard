                    ...
                    {specs.xValues.map((xVal, i) => {
                      ...
                      return (
                        <Group key={`line-path-${key}-point-${i}`}>
                          ...
                          {(isNaN(d[currentDrug]) && key == 'US') && <text x={i == 0 ? specs.xScale(d[specs.xKey]) :  specs.xScale(d[specs.xKey])} y={specs.yScale(0)-8} stroke={''} fill={''} fontSize={16} textAnchor={'middle'} fontWeight={'bold'}>{getSymbol(d[currentDrug])}</text>}
                          ...
                          {(isNaN(d[currentDrug]) && key != 'US') && <text x={i == 0 ? specs.xScale(d[specs.xKey]) :  specs.xScale(d[specs.xKey])} y={specs.yScale(0)-8} stroke={'lightblue'} fill={'lightblue'} fontSize={16} textAnchor={'middle'} fontWeight={'bold'}>{getSymbol(d[currentDrug])}</text>}
                          {(!isNaN(d[currentDrug]) && key != 'US') && <Circle cx={specs.xScale(d[specs.xKey])} cy={Number(d[currentDrug]) == 0 ? (specs.yScale(0)-14) : (specs.yScale(d[currentDrug]))} r={currentTimeframe === 'Monthly' ? 4: 5} fill={currentTimeframe === 'Monthly' && d[specs.xKey] == currentMonth ? 'orange' : UtilityFunctions.getSeriesColor(currentDrug, key)} />}
                        </Group>
                      )
                    })}
